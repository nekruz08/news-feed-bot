...
import (
	...
	"github.com/tomakado/containers/set" //библиотека для создания сети

	"github.com/nekruz08/news-feed-bot/internal/model"
	src "github.com/nekruz08/news-feed-bot/internal/source"
)
...
func (f *Fetcher) Fetch(ctx context.Context) error {
	sources, err := f.sources.Sources(ctx)// storage/source.go/ meth Sources/ `SELECT * FROM sources`
	if err != nil {
		return err
	}
	...
	for _, source := range sources {
		...
		go func(source Source) {
			...
			items, err := source.Fetch(ctx)//source/rss.go
			if err != nil {
...
// Хранит slice item в базе данных
...
// проверяем нет ли этой статьи в базе, если есть то пропускаем
