...
func main() {
	...
	var (
		...
		notifier       = notifier.New(articleStorage, botAPI, config.Get().NotificationInterval, 2*config.Get().FetchInterval, config.Get().TelegramChannelID)
	)
	...
	newsBot.RegisterCmdView("setpriority",middleware.AdminsOnly(config.Get().TelegramChannelID,bot.ViewCmdSetPriority(sourceStorage)))
	newsBot.RegisterCmdView("getsource",middleware.AdminsOnly(config.Get().TelegramChannelID,bot.ViewCmdGetSource(sourceStorage)))
	newsBot.RegisterCmdView("listsources",middleware.AdminsOnly(config.Get().TelegramChannelID,bot.ViewCmdListSource(sourceStorage)))
	...
	go func(ctx context.Context) {
		log.Printf("[INFO] HTTP server starting on port :8080")
